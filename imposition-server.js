...
async function apply8UpCenterStitchSheetwise(inputDoc, outputDoc, options = {}) {
   ...
   * ┌──────────────┬──────────────┐
   * │  Spread 1    │  Spread 3    │
   * │  N  |  1     │  N-2  |  3   │  ← Top row (odd spreads)
   * ├──────────────┼──────────────┤
   * │  Spread 2    │  Spread 4    │
   * │  N-1  |  2   │  N-3  |  4   │  ← Bottom row (even spreads - backs)
   * └──────────────┴──────────────┘
   */
  ...
  if (pageCount === 0) {
    console.log('⚠️ No pages to impose');
    return;
  }
  ...
  console.log(`\n📐 Sheet Dimensions: ${sheetWidth} × ${sheetHeight}`);
  console.log(`Layout: 2 spreads × 2 rows (4 spreads per sheet = 8 pages)`);
  ...
  for (let sheetIndex = 0; sheetIndex < totalSheets; sheetIndex++) {
    ...
    console.log(`  ✅ Sheet ${sheetNum} complete`);
  }
  
  console.log(`\n✅ 8-UP CENTER STITCH SHEETWISE COMPLETE`);
  console.log(`Total sheets: ${totalSheets}`);
  console.log(`Pages per sheet: 8 (4 spreads × 2 pages each)`);
}
...
async function apply8UpCenterStitchWorkTurn(inputDoc, outputDoc, options = {}) {
  ...
  console.log(`\n📖 Generated ${totalSpreads} spreads (Work & Turn)`);
  ...
  for (let sheetIndex = 0; sheetIndex < totalSheets; sheetIndex++) {
    ...
    console.log(`\n📄 Sheet ${sheetNum} of ${totalSheets} (Work & Turn):`);
    ...
    console.log(`  ✅ Sheet ${sheetNum} complete (Work & Turn)`);
  }
  
  console.log(`\n✅ 8-UP CENTER STITCH WORK & TURN COMPLETE`);
}
...
async function apply8UpCenterStitchWorkTumble(inputDoc, outputDoc, options = {}) {
  ...
  console.log(`\n📖 Generated ${totalSpreads} spreads (Work & Tumble)`);
  ...
  for (let sheetIndex = 0; sheetIndex < totalSheets; sheetIndex++) {
    ...
    console.log(`\n📄 Sheet ${sheetNum} of ${totalSheets} (Work & Tumble):`);
    ...
    console.log(`  ✅ Sheet ${sheetNum} complete (Work & Tumble)`);
  }
  
  console.log(`\n✅ 8-UP CENTER STITCH WORK & TUMBLE COMPLETE`);
}
...
async function applySignatureImposition(inputDoc, outputDoc, options = {}) {
  ...
  if (!signatureSize || signatureSize < 4 || signatureSize > 60 || signatureSize % 2 !== 0) {
    console.log(`⚠️ Invalid signature size: ${signatureSize}. Must be an even number between 4 and 60. Using 16-page signatures.`);
    signatureSize = 16;
  }
  ...
  if (blanksInLastSignature > 4 && remainingPages > 0) {
    console.log(`⚠️ Last signature would have ${blanksInLastSignature} blank pages (more than 4)`);
    ...
    if (bestBlanks > 0 && remainingPages >= 4 && remainingPages % 2 === 0) {
      console.log(`✅ Using exact remaining pages (${remainingPages}) for last signature (0 blanks)`);
      ...
      console.log(`✅ Rebalancing last signature from ${signatureSize} to ${bestSize} pages (${bestBlanks} blanks instead of ${blanksInLastSignature})`);
  ...
  console.log(`Sheet dimensions: ${sheetWidth} × ${sheetHeight} (2 pages per sheet)`);
  ...
  for (let sigIndex = 0; sigIndex < totalSignatures; sigIndex++) {
    ...
    console.log(`\n📄 Signature ${sigNum} (${currentSignatureSize} pages, starting at page ${basePageNum + 1}):`);
    ...
    console.log(`  ✅ Signature ${sigNum} complete`);
  }
  
  console.log(`\n✅ SIGNATURE IMPOSITION COMPLETE`);
...
function generateSignaturePageOrder(signatureSize, basePageNum) {
  ...
  if (signatureSize === 4) {
    ...
    console.log(`⚠️ Using dynamic page order generation for ${signatureSize}-page signature`);
...
// Start server
app.listen(PORT, () => {
  console.log(`🚀 Imposition Server running on http://localhost:${PORT}`);
