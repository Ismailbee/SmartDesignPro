<template>
        ...
        <div class="p-3 space-y-3" style="padding-top: 1rem; margin-top: 0; pointer-events: auto !important;">
          ...
          <div class="flex items-center gap-1">
            ...
            <input
              ...
              step="0.1"
              placeholder="5.7"
              class="w-16 px-1 py-0.5 text-xs border border-black dark:border-gray-600 rounded focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
              style="pointer-events: auto !important;"
          ...
          <div class="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-600">
            <label class="flex items-center gap-2 text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer mb-3">
              <input
                ...
                type="checkbox"
                class="rounded border-black cursor-pointer accent-cyan-500"
                style="pointer-events: auto !important;"
              />
            ...
            <div v-if="enableTextBackground" class="mb-2">
              ...
              <select 
                ...
                class="w-full px-2 py-1 text-xs border border-black dark:border-gray-600 rounded focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white"
                style="pointer-events: auto !important;"
              >
      ...
      <div v-if="windowWidth < 768" class="flex flex-col gap-1">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="flex-1 min-w-0">
            <h1 class="text-xs font-bold text-slate-900 dark:text-white truncate">Receipt Preview</h1>
          </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <div class="invoice-content-wrapper flex flex-col h-full w-full" :style="{ paddingTop: '0.1in', paddingBottom: '0.1in', paddingLeft: '0.12in', paddingRight: '0.12in', height: '100%', boxSizing: 'border-box', overflow: 'hidden' }">
         <!-- Header -->
        <div class="text-center flex-shrink-0" :style="{ marginBottom: '0.02in' }">
          ...
          <div class="flex ml-3 gap-3 items-start">
            ...
            <!-- Organization Info -->
            <div v-if="organizationName?.trim() || organizationSubName?.trim()" class="flex-1">
              ...
              <p 
                ...
                :style="{ 
                  fontSize: invoiceWidth <= 5 ? '11px' : '13px',
                  lineHeight: '1.0',
                  color: '#1f2937',
                  fontWeight: '800',
                  marginTop: '1px',
              ...
              <div v-if="headOfficeAddress?.trim() || headOfficePhone?.trim()" class="flex items-start justify-center mt-1">
          ...
          <div class="flex ml-5 justify-center items-center mt-[-1px] mb-1" :style="{ marginTop: '0.01in 0' }">
 ...
 <!-- Body -->
        <div class="text-sm flex-grow p-2 border-[2px] border-black rounded-xl" :style="{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '4px', height: `${Math.max(2.5, invoiceHeight - 2.5)}in`, overflow: 'hidden' }">
          ...
          <div class="flex justify-between items-end  mb-3" :style="{ paddingTop: '0px' }">
...
// PERFORMANCE: Lazy load heavy export libraries (saves ~1.2MB from initial bundle)
// These are only loaded when user actually exports
...
// Lazy load Capacitor plugins - only load when actually needed for save/share
const loadFilesystem = () => import('@capacitor/filesystem').then(m => ({ Filesystem: m.Filesystem, Directory: m.Directory }));
const loadShare = () => import('@capacitor/share').then(m => m.Share);
...
export default defineComponent({
  ...
  setup() {
    ...
    // Receipt-specific reactive variables
    ...
    // Size settings - Default to 6x4.5 inches
    const invoiceWidth = ref(6);
    const invoiceHeight = ref(4.5); // Default to 4.5 inches for 6x4.5 invoice
    ...
    const autoFitZoom = () => {
      nextTick(() => {
        // Find the correct container with multiple fallback options
        ...
        if (!container || !container.clientWidth || !container.clientHeight) {
          // Silently skip auto-fit if container is not ready yet
          return;
        }
  ...
  const showPreview = ref(true);
    ...
    const organizationSubFontSize = computed(() => {
      return '15px'; // Fixed size for stability
    });
    ...
    const exportGhostscriptCMYK = async (exportType = 'current', customName = null, format = 'pdf') => {
      ...
      try {
        ...
        if (format === 'pdf') {
          ...
          // PERFORMANCE: Load html2canvas only when needed
    ...
    const handleExportPDF = async (exportType = 'current', customName = null) => {
      ...
      try {
        ...
        // PERFORMANCE: Load htmlToImage only when needed
    ...
    const handleSaveReceipt = async () => {
      ...
      try {
        ...
        if (result.success) {
          ...
          alert(`⚠️ Receipt saved locally but Firebase save failed: ${result.error}\n\nYou can still access it from localStorage key: "ican_receipts_${member.branch}"`);
        }
      } catch (error) {
    ...
    const handlePaymentForOverflow = () => {
      // Optional: Add logic to handle text overflow between payment for fields
    };
    
    // Handle payment for 2 input
    const handlePaymentFor2Input = () => {
      // Optional: Add logic for paymentFor2 field
    };
    
    // Load signatures from Firebase
    const loadSignatures = async () => {
      try {
        console.log('🖋️ [PreviewIcanReceipt] Loading signatures...');
        ...
        if (!memberData) {
          console.log('🖋️ [PreviewIcanReceipt] No authenticated member data available');
          return;
        }
        ...
        try {
          ...
          console.log('🖋️ [PreviewIcanReceipt] Error parsing member data');
          return;
        }
        
        if (!member?.branch) {
          console.log('🖋️ [PreviewIcanReceipt] No branch found in member data');
          return;
        }

        console.log('🖋️ [PreviewIcanReceipt] Loading signatures for branch:', member.branch);
        ...
        console.log('🔥 [PreviewIcanReceipt] Firebase getAllSignatures result:', result);
        
        if (result?.success) {
          savedSignatures.value = result.signatures || result.data || [];
          console.log('✅ [PreviewIcanReceipt] Loaded signatures count:', savedSignatures.value.length);
          console.log('📝 [PreviewIcanReceipt] Signature details:', savedSignatures.value.map(s => ({ 
            id: s.id, 
            name: s.name,
          ...
          if (primary) {
            console.log('🖋️ [PreviewIcanReceipt] Setting primary signature:', primary.name);
            ...
            console.log('🖋️ [PreviewIcanReceipt] No primary signature, using first available');
          ...
          if (savedSignatures.value.length === 0) {
            console.log('🗃️ [PreviewIcanReceipt] No Firebase signatures, checking localStorage...');
            const signatureKeys = Object.keys(localStorage).filter(key => 
              key.includes('signature') || key.includes('Signature')
            );
            console.log('🗃️ [PreviewIcanReceipt] Found localStorage signature keys:', signatureKeys);
              ...
              try {
                ...
                signatureKeys.forEach(key => {
                  try {
                    ...
                    console.log('🗃️ [PreviewIcanReceipt] Failed to parse localStorage signature:', key);
                  }
                });
                
                if (localSignatures.length > 0) {
                  console.log('📦 [PreviewIcanReceipt] Using localStorage signatures:', localSignatures.length);
                  savedSignatures.value = localSignatures;
                }
              } catch (error) {
                console.error('🗃️ [PreviewIcanReceipt] Error processing localStorage signatures:', error);
              }
            }
          }
        } else {
          console.log('❌ [PreviewIcanReceipt] Failed to load signatures from Firebase');
          savedSignatures.value = [];
        }
      } catch (error) {
        console.error('❌ [PreviewIcanReceipt] Error in loadSignatures:', error);
        savedSignatures.value = [];
      }
    };
    ...
    const createNewReceipt = () => {
      if (confirm('Are you sure you want to start a new receipt? Any unsaved changes will be lost.')) {
        ...
        router.push({ name: 'ican-app-receipt', query: router.currentRoute.value.query });
      }
    };
...
.organization-name {
  ...
  /* Force font weight for APK */
  -webkit-font-feature-settings: 'kern' 1;
  font-feature-settings: 'kern' 1;
  /* Ensure bold rendering on Android */
