        ...
        <div class="p-3 space-y-3" style="padding-top: 1rem; margin-top: 0; pointer-events: auto !important;">
          ...
          <div class="flex items-center gap-1">
            ...
            <input
              ...
              step="0.1"
              placeholder="5.7"
              class="w-16 px-1 py-0.5 text-xs border border-gray-300 dark:border-gray-600 rounded focus:outline-none focus:ring-1 focus:ring-indigo-500 bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
              style="pointer-events: auto !important;"
      ...
      >
        ...
        <div class="text-center  flex-shrink-0" :style="{ paddingBottom: '4px' }">
          ...
          <div v-if="headOfficeAddress?.trim() || headOfficePhone?.trim()" class="flex items-start justify-center mt-1 gap-6">
            ...
            <div 
              ...
              class="text-left mt-0 min-w-[110px] border-l border-slate-300 pl-3"
        ...
        <div class="mt-auto" :style="{ fontSize: footerFontSize }">
          ...
          <div class="flex items-center h-7 gap-2">
            <div class="print-only flex-1 border-b border-dotted border-black" :style="{ borderBottom: `1px dotted ${cmykToRgbCss(cmykColors.black.c, cmykColors.black.m, cmykColors.black.y, cmykColors.black.k)} !important`, fontSize: '1.2em', fontFamily: 'Montserrat, sans-serif', fontWeight: '700', wordWrap: 'break-word', whiteSpace: 'pre-wrap' }">{{ sumOf2 ? sumOf2 + ' Only' : '' }}</div>
            <input
              ref="sumOfInput2"
              v-model="sumOf2"
              type="text"
...
import { loadFilesystem, loadShare } from '@/composables/useCapacitorPlugins';
...
export default defineComponent({
  ...
  const showPreview = ref(true);
    ...
    const organizationNameFontSize = computed(() => {
      ...
      // More aggressive size reduction for long names to ensure everything fits in 2 lines
      if (nameLength > 60) {
        baseSize = 18; // Very small for extremely long names
      } else if (nameLength > 50) {
        baseSize = 20; // Small for very long names
      } else if (nameLength > 40) {
        baseSize = 22; // Smaller for long names
      } else if (nameLength > 30) {
        baseSize = 25; // Medium-small for moderately long names
      ...
      const finalSize = Math.max(16, Math.min(44, scaledSize));
      return `${finalSize}px`;
    });
      ...
      try {
        ...
        if (isNative) {
          try {
            ...
            // For APK, show success without share dialog
          ...
          if (isNative) {
            try {
              ...
              try {
                ...
                alert('❌ Failed to save image to device. Please check app permissions in Settings.');
              }
            }
          } else {
            try {
