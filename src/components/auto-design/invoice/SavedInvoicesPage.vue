      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Saved Invoices</h1>
        ...
        >
            ...
            <div class="aspect-[3/4] bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <div class="p-3 text-xs">
                ...
                <div class="border-t border-b border-slate-200 py-1 mb-2">
                  <div class="text-center font-semibold">INVOICE #{{ invoice.invoiceNumber || 'N/A' }}</div>
                </div>
                
                <div class="space-y-1 text-slate-600">
                  <div><strong>Customer:</strong> {{ invoice.customerName || 'N/A' }}</div>
                  <div><strong>Date:</strong> {{ formatDate(invoice.invoiceDate) }}</div>
                  <div><strong>Total:</strong> {{ formatCurrency(invoice.grandTotal || 0) }}</div>
                </div>
              </div>
            </div>
          ...
          <div class="p-4">
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-3">
              <span>Items: {{ (invoice.items || []).filter(item => item.description?.trim()).length }}</span>
  ...
  setup() {
    ...
    const loadInvoices = async () => {
      ...
      try {
        ...
        try {
          ...
          const result = await getAllInvoices(member.branch);
          if (result.success) {
            invoices.value = result.invoices || [];
          } else {
            console.error('Firebase error:', result.error);
            // Fallback to localStorage
            const localInvoices = localStorage.getItem(`invoices_${member.branch}`);
            if (localInvoices) {
    ...
    const filteredInvoices = computed(() => {
      ...
      filtered.sort((a, b) => {
        switch (sortBy.value) {
          ...
          case 'dateAsc':
            return new Date(a.createdAt || a.invoiceDate) - new Date(b.createdAt || b.invoiceDate);
          case 'customerName':
            return (a.customerName || '').localeCompare(b.customerName || '');
          case 'invoiceNumber':
            return (a.invoiceNumber || '').toString().localeCompare((b.invoiceNumber || '').toString());
          default:
            return 0;
        }
      });
    ...
    const duplicateInvoice = (invoice) => {
      const duplicateData = {
        ...invoice,
        id: undefined,
        invoiceNumber: `${invoice.invoiceNumber || 'INV'}-COPY`,
    ...
    const deleteInvoice = async (invoice) => {
      if (!confirm(`Are you sure you want to delete invoice #${invoice.invoiceNumber || 'N/A'}?`)) {
        return;
      }
